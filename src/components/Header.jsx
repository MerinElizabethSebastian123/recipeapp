...
const Header = ({title, image, type}) => {
  return (
    ...
    bg-gradient-to-t from-black to-transparent top-0 z-8 flex flex-col items-center
    justify-center pt-40 2x1:pt-20 px-4 '>
        <h1 className ='text-white text-4x1 md:text-5x1 font-bold text-center'>{title}</h1>
        { 
        type && (
            <p className='text-sm mt-4 text-center
            text-green-500 bg- [#00000090] px-6 py-4 rounded-full'>Welcome! 
                <br className='hidden md:block'/>
                Discover a treasure trove of 
                deletcable recipes from around the globe
            </p>
        )
        }
